...
import utilities, {
    ...
    MessageUtil,
    startProgress,
    attachCancelablePromise,
    cancelAttachedPromise,
} from "../utils";
...
class BackupPage extends Component {
                ...
                return (
                    <div className={"Operations"}>
                        ...
                        <Button
                            type="primary"
                            disabled={
                                this.state.deleting.indexOf(record.id) >= 0 ||
                                (this.state.key !== record.id && this.state.restoring)
                            }
    ...
    deleteBackup = (index, record) => {
        ...
        this.setState({
            key: record.id,
            deleting: utilities.immutableAdd(this.state.deleting, record.id),
            progress: utilities.immutableAdd(this.state.progress, deleteAction),
        });
        dispatch(
            startProgress(
                deleteAction,
                { message: record.path },
                { name: intl.formatMessage(myMessages.deleteBackup), message: record.path }
            )
        )
            .then((ws) => {
                return attachCancelablePromise(
                    ajax.del(
                        AxiosUtil.getURL(AxiosUtil.getPath(WsPath.Resource.Backup, record.id), { progressKey: ws.key })
                    ),
                    this
                ).promise;
            })
            .then(() => {
                message.success(
                    MessageUtil.FormatMessage(intl, appMessage.actionSuccess, undefined, { action: appMessage.delete })
                );
                this.tableRef.current.refresh();
                this.setState({
                    key: 0,
                    deleting: utilities.immutableRemove(this.state.deleting, record.id),
                    progress: utilities.immutableRemove(this.state.progress, deleteAction),
                });
            })
            .catch((err) => {
                if (!err.isCanceled) {
    ...
    restore = (index, record) => {
        ...
        ajax.post(AxiosUtil.getPath(WsPath.Service.restore, record.id))
            .then(() => {
                message.success(
                    MessageUtil.FormatMessage(intl, appMessage.actionSuccess, undefined, { action: appMessage.restore })
                );
    ...
    createBackup = () => {
        ...
        dispatch(
            startProgress(BackupAction.create, undefined, {
                name: intl.formatMessage(myMessages.createBackup),
                message: formatProgress("backup.progress.backup"),
            })
        )
            .then((ws) => {
                return attachCancelablePromise(
                    ajax.post(WsPath.Resource.Backup, { progressKey: ws.key }, AxiosUtil.FormRequestConfig),
                    this
                ).promise;
            })
            .then(() => {
                let { intl } = this.props;
                message.success(
                    MessageUtil.FormatMessage(intl, appMessage.actionSuccess, undefined, { action: appMessage.create })
                );
                this.tableRef.current.refresh();
                this.setState({
                    creating: false,
                    progress: utilities.immutableRemove(this.state.progress, BackupAction.create),
                });
            })
            .catch((err) => {
                if (!err.isCanceled) {
        ...
        return (
            <div className={"Backup"}>
                <div className={"Toolbar"}>
                    <Button onClick={this.createBackup} loading={this.state.creating}>
                        <FormattedMessage {...myMessages.createBackup} />
                    </Button>
                    <Button onClick={this.refreshTable}>
                        <FormattedMessage {...appMessage.refresh} />
                    </Button>
                    {this.state.progress.length > 0 && (
                        <ProgressBar code={this.state.progress[0]} className={"Progress"} />
                    )}
                </div>
                <PagingTable
                    columns={this.columns}
                    dataPath={WsPath.Resource.Backup}
                ...
                <Prompt
                    ...
                    message={(localtion) => {
                        ...
                        if (action) {
                            return MessageUtil.FormatMessage(intl, appMessage.leaveWarning, undefined, {
                                action: action,
                            });
                        }
