...
class InstallPage extends Component {
    ...
    fileColumns = [{
        ...
        render : (text, record, index) => {
            ...
            if (text) {
                ...
                return <Tag color="lime"><FormattedMessage id={"module.not.install"}/></Tag>;
            }
        }
    }, {
        ...
        render : (text, record, index) => {
            switch (record.status) {
                case 0:
                    return <Tag color={"green"}><FormattedMessage id={"file.status.new"} defaultMessage={"Install"}/></Tag>;
                case 1:
                    return <Tag color={"blue"}><FormattedMessage id={"file.status.upgrade"} defaultMessage={"Upgrade"}/></Tag>;
                case 2:
                    return <Tag color={"volcano"}><FormattedMessage id={"file.status.existed"} defaultMessage={"Existed"}/></Tag>;
                case 3:
                    return <Tag color={"red"}><FormattedMessage id={"file.status.conflict"} defaultMessage={"Version Conflict"}/></Tag>;
            }
        }
    },
    ]
    ...
    install = () => {
        if (this.state.selectedRowKeys.length > 0) {
            ...
            ajax.post(WsPath.Resource.Module, {modules : postData}).then(data => {
                if(data && data.length) {
                    this.updateData(data);
                }
                message.success(MessageUtil.FormatMessage(intl, appMessage.actionSuccess, undefined, {action: appMessage.install}))
            }).catch(err => {
                message.error(formatErrorMessage(intl, err, MessageUtil.FormatMessage(intl, appMessage.actionFailed, undefined, {action : appMessage.install})))
            }).finally(() => {
    ...
    render() {
        return (
            <div className={"Install"}>
                <Dragger name="file" multiple={true} accept=".mapp" action={WsPath.Resource.File} customRequest={ajax.upload} onChange={this.onFileChange} disabled={this.state.installing}>
                    <p className="ant-upload-drag-icon">
                        <Icon type="inbox" />
                    </p>
                    <p className="ant-upload-text"><FormattedMessage id={"install.upload.description"} defaultMessage={"Click or drag file to this area to upload"}/></p>
                    <p className="ant-upload-hint"><FormattedMessage id={"install.upload.fileType"} defaultMessage={"Support *.mapp only"}/></p>
                </Dragger>
               <Table className={"File-Table"} loading={this.state.installing} rowKey={"key"} bordered size={"middle"} pagination={false} columns={this.fileColumns} dataSource={this.state.data} rowSelection={this.tableSelectOption()} />
                <div style={{textAlign: "center", marginTop: '2rem'}} className={"Footer"}>
                    <Button size={'large'} type={'primary'} loading={this.state.installing} disabled={this.state.data.length === 0 || this.state.selectedRowKeys.length === 0} onClick={this.install}><FormattedMessage id={"install.module.install"} defaultMessage={"Install/Upgrade"}/></Button>
                    <Button size={'large'} type={"warning"} disabled={this.state.data.length === 0 || this.state.installing} onClick={this.clear}><FormattedMessage id={"install.module.clear"} defaultMessage={"Clear"}/></Button>
                </div>
            </div>
        )
    }
