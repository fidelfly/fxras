            ...
            <Menu mode={"horizontal"} className={"App-Header-Menu"}>
                <Menu.Item key={"progress"}>
                    ...
                    <Drawer
                        title={
                            <div>
                                <FormattedMessage {...appMessage.backgroundProgress} />
                                {"( " + pks.length + " )"}
                            </div>
                        }
                        width={520}
                        ...
                        {pks.length === 0 && (
                            <div>
                                <FormattedMessage id={"progress.empty"} defaultMessage={"No Progress Running..."} />
                            </div>
                        )}
                        {pks.length > 0 && (
                            <div>
                ...
                <Menu.SubMenu
                    title={
                        <span>
                            {this.props.userInfo.avatar > 0 ? (
                                <Avatar
                                    className={"Avatar"}
                                    src={AxiosUtil.getPath(WsPath.PublicResource.Asset, this.props.userInfo.avatar)}
                                />
                            ) : (
                    ...
                    <Menu.Item key={"logout"}>
                        <Link to={"/logout"}>
                            <Icon type={"logout"} />
                            <span>
                                <FormattedMessage {...appMessage.logout} />
                            </span>
                        </Link>
                    </Menu.Item>
                </Menu.SubMenu>
